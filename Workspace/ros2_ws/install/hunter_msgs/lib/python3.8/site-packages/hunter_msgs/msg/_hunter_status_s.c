...
bool hunter_msgs__msg__hunter_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // motor_states
    ...
    Py_ssize_t size = 3;
    hunter_msgs__msg__HunterMotorState * dest = ros_message->motor_states;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!hunter_msgs__msg__hunter_motor_state__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * hunter_msgs__msg__hunter_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // motor_states
    ...
    hunter_msgs__msg__HunterMotorState * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->motor_states[i]);
      PyObject * pyitem = hunter_msgs__msg__hunter_motor_state__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
